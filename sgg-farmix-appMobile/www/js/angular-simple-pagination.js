...
function SimplePagination() {
  return {
    ...
    template: '\n      <div class="simple-pagination">\n        <!--<p class="simple-pagination__items">Mostrando {{pagination.pageLimit}} de {{pagination.total}}</p>-->\n        <p>\n          <button ng-click="pagination.previousPage()" ng-disabled="pagination.currentPage <= 0" class="simple-pagination__button simple-pagination__button--prev" ng-class="{disabled: pagination.currentPage <= 0}">\n            Anterior\n          </button>\n          <span class="simple-pagination__pages">{{pagination.currentPage + 1}} de {{pagination.getTotalPages()}}</span>\n          <button ng-click="pagination.nextPage()" ng-disabled="pagination.currentPage === (pagination.getTotalPages() - 1)" class="simple-pagination__button simple-pagination__button--next" ng-class="{disabled: pagination.currentPage === (pagination.getTotalPages() - 1)}">\n            Siguiente\n          </button>\n        </p>\n        <p class="simple-pagination__page-limit">\n          <span class="simple-pagination__page-limit__option" ng-repeat="limit in pagination.pageLimits" ng-if="limit < pagination.total">\n            <!--<a href="" ng-click="pagination.setItemsPerPages(limit)" ng-class="{\'active\': pagination.isCurrentPageLimit(limit)}">{{limit}}</a>\n          </span>\n          <span>\n            <a href="" ng-click="pagination.setItemsPerPages(pagination.total)" ng-class="{\'active\': pagination.isCurrentPageLimit(pagination.total)}">All</a>-->\n          </span>\n        </p>\n      </div>\n    '
  };
}
