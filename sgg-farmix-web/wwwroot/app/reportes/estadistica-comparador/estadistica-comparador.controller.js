(function () {
    ...
    function estadisticaComparadorController($location, $scope, $sessionStorage, estadisticaComparadorService, toastr, estadisticaBovinoService, estadisticaEventoService, estadisticaInseminacionService) {
        ...
        function descargarPdf(idDeImagen, titulo, orientacion) {
            var orientacionDePagina, anchoImagen, altoImagen, margenIzquierdoLogoFarmix, campo;
            if (orientacion === 'landscape') {
                orientacionDePagina = 'landscape';
                anchoImagen = 1000;
                altoImagen = 500;
                margenIzquierdoLogoFarmix = 230;
            } else {
                orientacionDePagina = 'portrait';
                anchoImagen = 500;
                altoImagen = 250;
                margenIzquierdoLogoFarmix = 100;
            }
            var imgSrc = document.getElementById(idDeImagen).href;
            var fecha = new Date();
            if (idDeImagen.indexOf('1') > 0) {
                for (var i = 0; i < vm.campos1.length; i++) {
                    if (vm.campos1[i].codigoCampo === parseInt(vm.filtro.campo1)) {
                        campo = vm.campos1[i].nombre;
                        break;
                    }
                }
            }
            else if (idDeImagen.indexOf('2') > 0) {
                for (var i = 0; i < vm.campos2.length; i++) {
                    if (vm.campos2[i].codigoCampo === parseInt(vm.filtro.campo2)) {
                        campo = vm.campos2[i].nombre;
                        break;
                    }
                }
            }
            var docDefinition = {
                pageSize: 'A4',
                pageOrientation: orientacionDePagina,
                content: [
                    {
                        ...
                        width: 280,
                        height: 70,
                        margin: [margenIzquierdoLogoFarmix, 5]
                    },
                    {
                        table: {
                            widths: ['*'],
